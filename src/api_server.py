...
def regenerate_article(req: RegenerateRequest, username: str = Depends(get_current_username)):
    # Prefix hack to signal force-rewrite
    topic_with_prefix = f"REGENERATE:{req.topic}"
    # High priority to start immediately
    scheduler.add_or_update_task(topic_with_prefix, priority=10)
    return {"message": f"Re-generation scheduled for '{req.topic}'."}

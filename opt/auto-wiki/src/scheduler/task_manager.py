...
class WikiScheduler:
    def __init__(self, db_path="/app/scheduler.db"):
    ...
    def fetch_external_trends(self):
        ...
        print("🌍 Fetching external trends...")
        # Google Trends Daily RSS (Japan)
    ...
    def add_or_update_task(self, topic: str, priority: int = 5, volatility_days: int = 1):
        ...
        if row:
            # 既に存在する場合、完了済みなら再スケジュールを検討（ここでは単純に優先度更新のみ）
            # cursor.execute("UPDATE tasks SET priority = ? WHERE id = ?", (priority, row[0]))
            conn.commit()
            conn.close()
            return False
        else:
